const { EmployerDriverManagement } = require('../models');
const { to, ReE, ReS } = require('../services/util.service');

const create = async function(req, res) {
    res.setHeader('Content-Type', 'application/json');
    let err, employerDriverManagement;
    // let user = req.user;
    let employer_driver_management_info = req.body;
    [err, employerDriverManagement] = await to(EmployerDriverManagement.create(employer_driver_management_info));
    if (err) return ReE(res, err, 422);
    return ReS(res, { employerDriverManagement: employerDriverManagement.toWeb() }, 201);
}
